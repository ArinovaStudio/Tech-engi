...
async function main() {
  ...
  */
  ...
  if (clientProfile) {
    await prisma.project.create({
      data: {
        ...
        description:
          "Need a microcontroller-based system to read DHT11 sensors and control AC units via relays.",
        budget: 10000,
        instruments: [
          "Arduino",
          "DHT11",
    ...
    await prisma.project.create({
      data: {
        ...
        description:
          "Looking for an expert to write custom PID loops for a quadcopter using STM32.",
        budget: 25000,
        instruments: [
          "STM32",
          "IMU MPU6050",
    ...
    await prisma.project.create({
      data: {
        ...
        description:
          "Design a 4-layer PCB for a smartwatch prototype.",
        budget: 10000,
        instruments: [
          "Altium",
          "Eagle",
  ...
  for (let i = 1; i <= 30; i++) {
    const shuffledSkills = skillPool.sort(
      () => 0.5 - Math.random()
    );

    const engineerSkills = shuffledSkills.slice(
      0,
...
await prisma.user.upsert({
  ...
  create: {
    name: "Aditya Singh",
    email: "aditya.engineer@gmail.com",
    password: hashedPassword,
    role: Role.ENGINEER,
    emailVerified: new Date(),
    engineerProfile: {
