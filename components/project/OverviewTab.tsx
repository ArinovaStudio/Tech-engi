...
export default function OverviewTab({ project }: { project: any }) {
  ...
  return (
    <>
      ...
      <div className="bg-gray-50 dark:bg-background" style={{ padding: "1.5rem", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
          ...
          <div>
            ...
            {project.designSystem && (
              <div id="design-system" className="mt-6">
                <h2 className="text-lg font-bold text-gray-900 mb-5 dark:text-slate-100">
                  Design System
                </h2>
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-5">
                    {/* Brand */}
                    <div className="bg-white border border-gray-200 rounded-xl p-5 dark:bg-card dark:border-slate-800">
                      <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 dark:text-slate-400">Brand Name</p>
                      <p className="text-base font-semibold text-gray-900 dark:text-slate-100">{project.designSystem.brandName || "-"}</p>
                    </div>
                    {/* Colors */}
                    {project.designSystem.colors?.length > 0 && (
                      <div className="bg-white border border-gray-200 rounded-xl p-5 dark:bg-card dark:border-slate-800">
                        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3 dark:text-slate-400">Color Palette</p>
                    ...
                    {/* Fonts */}
                    <div className="bg-white border border-gray-200 rounded-xl p-5 dark:bg-card dark:border-slate-800">
                      <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3 dark:text-slate-400">Typography</p>
                      <div className="space-y-3">
                        <div className="grid grid-cols-[120px_1fr]">
                          <span className="text-xs font-semibold text-gray-600 dark:text-slate-400">Primary Font</span>
                          <span className="text-sm text-gray-900 dark:text-slate-100">{project.designSystem.fonts?.primary || "-"}</span>
                        </div>
                        <div className="grid grid-cols-[120px_1fr]">
                          <span className="text-xs font-semibold text-gray-600 dark:text-slate-400">Secondary Font</span>
                          <span className="text-sm text-gray-900 dark:text-slate-100">{project.designSystem.fonts?.secondary || "-"}</span>
                        </div>
                      </div>
                    </div>
                    {/* Key Pages */}
                  ...
                  <div className="space-y-5">
                    ...
                    {project.designSystem.layoutStyle && (
                      <div className="bg-white border border-gray-200 rounded-xl p-5 dark:bg-card dark:border-slate-800">
                        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3 dark:text-slate-400">Layout Style</p>
                        <div className="space-y-2">
                          {Object.entries(project.designSystem.layoutStyle).map(([key, value]: any) => (
                            <div key={key} className="grid grid-cols-[160px_1fr] items-center px-3 py-2 rounded-lg border border-gray-100 dark:border-slate-800">
                              <span className="text-xs font-semibold text-gray-700 capitalize dark:text-slate-300">{key}</span>
                              <span className="text-sm text-gray-900 dark:text-slate-100">{String(value)}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {project.designSystem.visualGuidelines && (
                      <div className="bg-white border border-gray-200 rounded-xl p-5 dark:bg-card dark:border-slate-800">
                        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3 dark:text-slate-400">Visual Guidelines</p>
                        <div className="space-y-2">
          ...
          <div id="project-summary-card" style={cardStyle}>
            ...
            {project.technology && project.technology.length > 0 && (
              <div id="technology-used" className="mt-4">
                <h2 className="text-base font-bold text-gray-900 mb-4 dark:text-slate-100">Technology Stack</h2>
                <div className="flex flex-wrap gap-3">
                  {project.technology.map((t: any, i: number) => (
                    <div key={i} className="border border-gray-300 rounded-lg px-3 py-2 min-w-[120px] dark:border-slate-700">
                      <p className="text-xs font-semibold text-gray-500 dark:text-slate-400">{t.area}</p>
                      <p className="text-sm font-semibold text-gray-900 dark:text-slate-100">{t.tech}</p>
                    </div>
                  ))}
                </div>
              </div>
