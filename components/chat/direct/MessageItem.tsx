    ...
    >
      ...
      {!isMine && !isSelectMode && (
        <div className="flex flex-col justify-end pb-5">
          <div className="w-8 h-8 rounded-full bg-gray-200 dark:bg-slate-700 shrink-0 overflow-hidden border border-gray-100 dark:border-slate-800">
            {msg.sender?.image ? (
      ...
      <div className={`flex flex-col max-w-[75%] ${isMine ? "items-end" : "items-start"}`}>
        ...
        <div className={`flex items-center gap-2.5 ${isMine ? "flex-row-reverse" : "flex-row"}`}>
          <div className={`rounded-3xl px-5 py-2.5 text-[15px] shadow-sm transition-all ${
            isMine 
              ...
              : "bg-white dark:bg-card text-[var(--text-primary)] border border-gray-100 dark:border-slate-700"
          ...
          {canModify && !msg.isPending && !msg.isDeleted && !isSelectMode && (
            <div className="relative opacity-0 group-hover:opacity-100 transition-opacity">
              <button 
                ...
                className="w-7 h-7 rounded-full bg-gray-100 dark:bg-slate-800 flex items-center justify-center text-gray-500 dark:text-slate-400 hover:text-gray-800 dark:hover:text-slate-200 hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors shadow-sm border border-gray-200 dark:border-slate-700"
              >
                <ChevronDown size={14} />
              </button>
              ...
              {/* Dropdown Menu */}
              {showMenu && (
                <div className={`absolute top-full mt-2 ${isMine ? "right-0" : "left-0"} bg-white dark:bg-card shadow-[0_4px_20px_rgb(0,0,0,0.08)] border border-gray-100 dark:border-slate-700 rounded-2xl py-2 w-32 z-50 flex flex-col overflow-hidden`}>
                  {isMine && (
                    <button onClick={(e) => { e.stopPropagation(); setShowMenu(false); onEdit(msg); }} className="flex items-center gap-3 px-4 py-2 text-[14px] hover:bg-gray-50 dark:hover:bg-slate-700 text-[#334155] dark:text-slate-200 transition-colors font-medium">
        ...
        {/* Timestamps & Ticks */}
        <div className={`text-[11px] mt-1.5 flex items-center gap-1.5 px-2 font-medium ${isMine ? "justify-end text-gray-500 dark:text-slate-400" : "justify-start text-gray-500 dark:text-slate-400"}`}>
