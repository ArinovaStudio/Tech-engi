...
const DailyTask = ({ projectId }: { projectId: string }) => {
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto space-y-8">
            {/* Form */}
            <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm dark:bg-card dark:border-slate-800">
                <div className="flex flex-wrap gap-6 items-end text-black dark:text-white">
                    {/* Title */}
                    <div className="flex-1 min-w-[220px]">
                        <label className="block text-xs uppercase tracking-widest font-semibold text-gray-500 mb-1.5 dark:text-slate-400">
                            Task
                        </label>
                        <input
                            type="text"
                            ...
                            className="w-full px-5 py-4 border border-gray-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 rounded-3xl outline-none text-base transition-all dark:border-slate-700"
                        />
                    </div>
                    ...
                    <div className="min-w-[170px]">
                        ...
                        <input
                            ...
                            className="w-full px-5 py-4 border border-gray-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 rounded-3xl outline-none transition-all dark:border-slate-700"
                        />
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        {editingId && (
                            <button
                                onClick={cancelEdit}
                                className="px-8 py-4 text-gray-700 hover:bg-gray-100 border border-gray-300 font-medium rounded-3xl transition-colors dark:text-slate-300 dark:border-slate-700"
                            >
                                Cancel
                            </button>
                        )}
            ...
            {sortedDates.length === 0 ? (
                <div className="bg-white border border-dashed border-gray-300 rounded-3xl py-16 text-center dark:bg-card dark:border-slate-700">
                    ...
                    <p className="text-xl font-medium text-gray-400 dark:text-slate-500">
                        No daily tasks yet
                    </p>
                    ...
                    {sortedDates.map((dateKey) => (
                        <div key={dateKey}>
                            ...
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <span className="text-xs font-medium text-gray-400 bg-white px-3 py-1 border border-gray-200 rounded-3xl dark:text-slate-500 dark:bg-card dark:border-slate-800">
                            ...
                            <div className="space-y-4">
                                {grouped[dateKey].map((task) => {
                                    ...
                                    return (
                                        <div
                                            key={task.id}
                                            className={`group bg-white border rounded-3xl p-6 flex gap-6 items-start transition-all duration-200 ${canEdit ? "border-gray-200 hover:border-gray-300 hover:shadow-md" : "border-gray-200 bg-gray-50" } dark:bg-card`}
                                        >
                                            <div className="flex-1 min-w-0">
                                                <p className="text-xl font-semibold text-gray-900 leading-tight break-words dark:text-slate-100">
                                                    {task.title}
                                                </p>
                                                {task.description && (
                                                    <p className="mt-3 text-gray-600 text-[15px] leading-relaxed dark:text-slate-400">
                                                        {task.description}
                                                    </p>
                                                )}
                                                ...
                                                {!canEdit && (
                                                    <div className="mt-2 flex items-center gap-1 text-xs text-gray-500 bg-gray-100 w-fit px-2 py-1 rounded dark:text-slate-400 dark:bg-slate-800">
                                                        <Lock size={12} />
                                            ...
                                            <div className="flex flex-col sm:flex-row items-center gap-1 opacity-50 group-hover:opacity-100 transition-opacity">
                                                {(isOwnTask && canEdit) || isAdmin ? (
                                                    ...
                                                    <div className="flex items-center gap-2 px-3 py-1 bg-gray-100 rounded-2xl dark:bg-slate-800">
                                                        <Lock size={16} className="text-gray-500 dark:text-slate-400" />
                                                        <span className="text-xs text-gray-600 font-medium dark:text-slate-400">Locked</span>
                                                    </div>
                                                ) : null}
                                            </div>
                                        </div>
