    ...
    <>
      <div className="p-6 border border-[var(--border)] rounded-2xl bg-white dark:bg-card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
          <div><p className="text-xs text-gray-500 uppercase dark:text-slate-400">Total Projects</p><p className="font-bold text-sm">{profile?.totalProjects || 0}</p></div>
          <div><p className="text-xs text-gray-500 uppercase dark:text-slate-400">Total Budget Spent</p><p className="font-bold text-sm">₹{(profile?.totalBudget || 0).toLocaleString()}</p></div>
          
          <div className="md:col-span-2 border-t pt-4">
            <p className="text-xs text-gray-500 uppercase mb-2 dark:text-slate-400">Areas of Expertise</p>
            <div className="flex flex-wrap gap-2">
              {profile?.expertise?.map((s: string) => <span key={s} className="bg-gray-100 border text-xs font-semibold px-3 py-1.5 rounded-md dark:bg-slate-800">{s}</span>)}
            </div>
          </div>
        </div>
      </div>
